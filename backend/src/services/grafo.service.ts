...
const procesarGrafo = (records: any[]) => {
...
export const getGrafoDatos = async () => {
  ...
  try {
    const result = await session.run(`
      ...
      LIMIT 800
    `);
    return procesarGrafo(result.records);
  } finally {
    await session.close();
  }
};

export const getGrafoRedCouriers = async () => {
  const session = getSession();
  try {
    const result = await session.run(`
      MATCH (n)-[r]->(m)
...
export const getGrafoCadenaProductos = async () => {
  const session = getSession();
  try {
    const result = await session.run(`
      MATCH (n)-[r]->(m)
...
export const getGrafoFlujoClientes = async () => {
  const session = getSession();
  try {
    const result = await session.run(`
      MATCH (n)-[r]->(m)
